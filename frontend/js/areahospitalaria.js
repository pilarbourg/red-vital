document.addEventListener('DOMContentLoaded', () => {
  ...
  const notificationDonorResultsBody = document.getElementById('notificationDonorResultsBody');
  const notificationSearchBtn = document.querySelector('.send-right .btn-search');
  ...
  const notificationBloodGroupSelect = document.querySelector('.send-right select');
  ...
  document.getElementById("updateBtn").addEventListener("click", async () => {
    ...
    for (let select of selects) {
      ...
      const res = await fetch(`http://localhost:3000/api/solicitud/${solicitudId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json"},
        body: JSON.stringify({ estado })
      });

      if (!res.ok) console.error(`Error al actualizar solicitud ${solicitudId}`);
    }
  ...
  notificationSearchBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      donantes.forEach(d => {
        ...
        tr.innerHTML = `
          <td><input type="checkbox" class="select-donor" data-donante-id="${d.id}"></td>
  ...
  notificationDonorResultsBody.addEventListener('change', e => {
    if (e.target.classList.contains('select-donor')) {
      const anySelected = notificationDonorResultsBody.querySelectorAll('.select-donor:checked').length > 0;
  ...
  notificationSendBtn.addEventListener('click', async () => {
    const selectedCheckboxes = notificationDonorResultsBody.querySelectorAll('.select-donor:checked');
    if (selectedCheckboxes.length === 0) {
      showToast("Por favor selecciona al menos un donante antes de enviar", "warning");
      return;
    }
    ...
    try {
      for (const checkbox of selectedCheckboxes) {
        ...
        const res = await fetch("http://localhost:3000/api/donantes/notificar", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            usuario_id: parseInt(donanteId),
            grupo_sanguineo: notificationBloodGroupSelect.value,
            mensaje: mensaje || `Se requiere donación urgente del tipo ${notificationBloodGroupSelect.value}`
          })
        });

        if (!res.ok) throw new Error(`Error enviando notificación al donante ${donanteId}`);
      }
  ...
  document.querySelector(".blood-request-left button").addEventListener("click", async () => {
    ...
    const res = await fetch("http://localhost:3000/api/hospitales/2/solicitud", { // ID hospital fijo o traer luego de login
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  async function loadSolicitudes() {
    try {
      const res = await fetch('http://localhost:3000/api/hospitales/2/solicitudes'); 
      ...
      solicitudes.forEach(solicitud => {
        ...
        const fechaLimite = solicitud.fecha_limite ? new Date(solicitud.fecha_limite).toLocaleDateString() : '-';
        ...
        tr.innerHTML = `
          ...
          <td>
            <select class="estado-select" data-solicitud-id="${solicitud.id}">
              ...
              <option value="PARCIAL" ${solicitud.estado === 'PARCIAL' ? 'selected' : ''}>Parcial</option>
              <option value="CUBIERTA" ${solicitud.estado === 'CUBIERTA' ? 'selected' : ''}>Cubierta</option>
  ...
  document.querySelector(".btn-create.btn-search").addEventListener("click", async () => {
    const grupo = document.getElementById("grupo-sanguineo-filter").value.trim();
    const nombre = document.getElementById("nombre-donante-filter").value.trim();
...
function applyColor(select, estado) {
  ...
  select.style.color =
    estado === 'PENDIENTE' ? '#d90429' :
    estado === 'PARCIAL' ? '#ffb703' :
    estado === 'CUBIERTA' ? '#2a9d8f' :
    estado === 'CANCELADA' ? '#666666' :
    '#000000';
}
